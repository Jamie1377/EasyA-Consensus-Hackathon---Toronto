    ...
    def run(self):
        try:
            self.backtester = AptosBacktester(
                symbols=self.symbol, initial_capital=self.initial_capital
            )
...
class FlexibleDashboard(QMainWindow):
    ...
    def setup_dashboard_tab(self):
        ...
        self.market_symbol.addItems(
            ["BTC-USD", "ETH-USD", "APT-USD", "AAPL", "MSFT", "TSLA"]
        )
        ...
        self.watchlist.setHorizontalHeaderLabels(
            ["Symbol", "Price", "Change", "Volume", "Actions"]
        )
    ...
    def setup_backtest_tab(self):
        ...
        self.backtest_symbol.addItems(
            ["BTC-USD", "ETH-USD", "APT-USD", "AAPL", "MSFT", "TSLA", "SPY"]
        )
        ...
        self.strategy_type.addItems(
            ["Moving Average Crossover", "RSI", "Bollinger Bands", "Custom"]
        )
        ...
        self.backtest_return = self.create_metric_widget(
            "Total Return", "0.00%", "blue"
        )
        ...
        self.backtest_drawdown = self.create_metric_widget(
            "Max Drawdown", "0.00%", "red"
        )
        ...
        self.backtest_trades.setHorizontalHeaderLabels(
            ["Date", "Type", "Price", "Size", "Value", "P&L"]
        )
        self.backtest_trades.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
    ...
    def populate_sample_watchlist(self):
        ...
        for i, (symbol, price, change, volume) in enumerate(
            zip(symbols, prices, changes, volumes)
        ):
    ...
    def update_portfolio_chart(self):
        ...
        # Format dates nicely
        self.portfolio_chart.axes.xaxis.set_major_formatter(
            mdates.DateFormatter("%Y-%m-%d")
        )
        self.portfolio_chart.axes.xaxis.set_major_locator(
    ...
    def show_deposit_dialog(self):
        ...
        QMessageBox.information(
            self, "Deposit Funds", "This feature is not implemented in the demo."
        )

    def show_withdraw_dialog(self):
        """Show withdrawal dialog"""
        QMessageBox.information(
            self, "Withdraw Funds", "This feature is not implemented in the demo."
        )
    ...
    def run_backtest(self):
        ...
        self.log(
            f"Starting backtest for {symbol} from {start_date} to {end_date}", "system"
        )
    ...
    def display_backtest_results(self, history, metrics):
        ...
        self.backtest_return.findChild(QLabel).setText(
            f"{metrics.get('total_return', 0):.2%}"
        )
        self.backtest_sharpe.findChild(QLabel).setText(
            f"{metrics.get('sharpe', 0):.2f}"
        )
        self.backtest_drawdown.findChild(QLabel).setText(
            f"{metrics.get('max_drawdown', 0):.2%}"
        )
        self.backtest_win_rate.findChild(QLabel).setText(
        ...
        self.log(
            f"Backtest completed with {metrics.get('total_return', 0):.2%} return",
            "system",
        )
    ...
    def update_backtest_trades(self):
        ...
        for i, (tx_type, symbol, price, qty, timestamp) in enumerate(transactions):
            self.backtest_trades.setItem(
                i, 0, QTableWidgetItem(timestamp.strftime("%Y-%m-%d"))
            )
            ...
            if i > 0 and tx_type == "SELL" and transactions[i - 1][0] == "BUY":
                prev_price = transactions[i - 1][2]
                pnl = (price - float(prev_price) * qty)
    ...
    def submit_order(self):
        ...
        price = (
            ...
            else float(self.current_price.text().replace("$", ""))
        )

        self.log(
            f"Submitting {side} order for {quantity} {symbol} at ${price:.2f}",
            "trading",
        )
        ...
        QMessageBox.information(
            self,
            "Order Submitted",
            f"Your {side} order for {quantity} {symbol} has been filled at ${price:.2f}",
        )
    ...
    def update_positions(self, symbol, side, quantity, price):
        ...
        for row in range(self.positions_table.rowCount()):
            if self.positions_table.item(row, 0).text() == symbol:
                ...
                entry_price = float(
                    self.positions_table.item(row, 3).text().replace("$", "")
                )
                ...
                if side == "Buy":
                    ...
                    new_entry = (
                        (current_size * entry_price) + (quantity * price)
                    ) / new_size
                    self.positions_table.setItem(
                        row, 3, QTableWidgetItem(f"${new_entry:.2f}")
                    )
                else:  # Sell
                    new_size = current_size - quantity
                ...
                if new_size <= 0:
                    ...
                    self.positions_table.setItem(
                        row, 2, QTableWidgetItem(f"{new_size:.6f}")
                    )
                    ...
                    self.positions_table.setItem(
                        row, 5, QTableWidgetItem(f"${pnl:.2f} ({pnl_pct:.2f}%)")
                    )

                    # Color code P&L
                    if pnl > 0:
                        self.positions_table.item(row, 5).setBackground(
                            QColor(200, 255, 200)
                        )
                    else:
                        self.positions_table.item(row, 5).setBackground(
    ...
    def execute_transfer_action(self):
        ...
        QMessageBox.information(
            self,
            "Transfer Complete",
            f"Successfully transferred {amount:.3f} APT to {recipient}",
        )
